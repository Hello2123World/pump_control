...
class DDPGAgent:
    ...
    def train_agent(self, file_path, pressure, n_epochs=20, max_steps_per_epoch=100, action_noise_scale=0.1,
                    noise_decay=0.99, min_noise_scale=0.01):
        """训练DDPG网络"""
        # 读取数据并初始化环境
        data = pd.read_csv(file_path)  # 输入的数据文件路径
        model_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'bp_net')
        current_dir = os.path.dirname(os.path.abspath(__file__))
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        # BP网络
        model_path = os.path.join(model_dir, f'BP_parameters_{pressure}.pth')
        action_scaler_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                          f'./scaler/action_normalizer_{pressure}.pkl')
        state_scaler_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                         f'./scaler/state_normalizer_{pressure}.pkl')
        ...
        # 保存模型
        actor_path = os.path.join(current_dir, f"./DDPG_net/actor_{pressure}.pth")
        critic_path = os.path.join(current_dir, f"./DDPG_net/critic_{pressure}.pth")
