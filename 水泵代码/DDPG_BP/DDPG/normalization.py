...
class DataNormalizer:
    def __init__(self, pressure,index):
        """
    ...
    def normalize_data(self, file_path):
        ...
        output_file_path = os.path.join(current_directory, f'./data/0.35/normalized_{self.pressure}_{self.index}.csv')
        new_data.to_csv(output_file_path, index=False)
        # print("new_data = ", new_data)

        directory = f'./scaler/{self.pressure}'
        # 检查目录是否存在，如果不存在，则创建
        ...
        action_scaler_path = os.path.join(current_directory,f'./scaler/{self.pressure}', f'action_{self.pressure}_{self.index}.pkl')
        state_scaler_path = os.path.join(current_directory, f'./scaler/{self.pressure}',f'state_{self.pressure}_{self.index}.pkl')
...
if __name__ == "__main__":
    ...
    # 输入文件路径，执行归一化处理
    file_path = f'./data/0.35/{pressure}_{index}.csv'  # 输入您的文件路径
